function scr_player_ratmount()
{
	...
	if ((check_wall(x + xscale, y) && !place_meeting(x + hsp, y, obj_destructibles)) or (abs(movespeed) < 8 && move != xscale) or !key_attack or abs(movespeed) <= 6)
	{
		gustavodash = 0;
	...
	if (ratmount_movespeed >= 12 or gusdashpadbuffer > 0)
	{
	...
	if (abs(movespeed) > 2)
		...
		image_speed = 0.35;
	if (sprite_index == spr_player_ratmountmach3 or sprite_index == spr_lonegustavo_mach3)
		image_speed = 0.4;
	...
	if (input_buffer_jump > 0 && can_jump && gusdashpadbuffer == 0 && state != states.ratmountskid)
	{
		...
		if (brick)
		{
			if ((ratmount_movespeed >= 12 && key_attack) or gusdashpadbuffer > 0)
				sprite_index = spr_player_ratmountdashjump;
			else
				sprite_index = spr_player_ratmountjump;
		}
		else if ((ratmount_movespeed >= 12 && key_attack) or gusdashpadbuffer > 0)
	...
	if (((input_buffer_slap > 0 && key_up) or key_shoot2) && brick && gusdashpadbuffer == 0)
	{
		input_buffer_slap = 0;
		ratmount_kickbrick();
	...
	if ((key_down && grounded && vsp > 0 && gusdashpadbuffer <= 0) or scr_solid(x, y))
	{
