...
else if (wastedhits == 9 && phase == 1 && !pizzahead && (other.instakillmove or other.state == states.handstandjump) && state == states.contemplate)
	scr_boss_do_hurt_phase2(other);
else if (state == states.mini && ministate != states.transition && (other.instakillmove or other.state == states.handstandjump))
{
	with (other)
		scr_pummel();
	with (obj_camera)
