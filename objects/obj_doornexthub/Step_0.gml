...
{
	if (key && !instance_exists(obj_jumpscare) && obj_player2.state == 0 && obj_player2.grounded && obj_player2.key_up && place_meeting(x, y, obj_player2))
	{
		_save = true;
		ds_list_add(global.saveroom, id);
		obj_player2.state = 98;
...
if (sprite_index == spr_elevatoropening && floor(image_index) == (image_number - 1) && place_meeting(x, y, obj_player1) && !instance_exists(obj_jumpscare) && floor(obj_player1.image_index) == (obj_player1.image_number - 1) && obj_player1.state == 98)
{
	with (obj_player1)
	{
