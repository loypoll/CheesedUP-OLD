...
with (obj_player)
{
	...
	{
		if (((key_down && !place_meeting(x, y + 1, obj_destructibles) && place_meeting(x, y + 1, other) && ((state == states.crouch || character == "S" || character == "M") || state == 65 || (state == states.tumble && sprite_index == spr_dive))) || ((state == 102 || (state == states.tumble && key_down) || state == states.freefall || state == 111) && !place_meeting(x, y + 1, obj_destructibles) && place_meeting(x, y + 1, other))) && !instance_exists(obj_fadeout) && state != states.door && state != states.comingoutdoor)
		{
	...
	if (other.image_yscale == -1)
	{
		if (((key_up && !place_meeting(x, y - 1, obj_destructibles) && place_meeting(x, y - 10, other) && (state == states.normal || state == states.pogo || state == states.jump || state == states.mach1 || state == states.mach2 || state == 121 || state == states.Sjumpprep || (state == states.punch && sprite_index == spr_player_breakdanceuppercut))) || ((state == states.Sjump || state == 123) && !place_meeting(x, y - 1, obj_destructibles) && place_meeting(x, y - 1, other))) && !instance_exists(obj_fadeout) && state != states.door && state != states.comingoutdoor)
		{
			obj_player1.lastroom = room;
			obj_player2.lastroom = room;
			fmod_event_one_shot_3d("event:/sfx/pep/box", x, y);
